...
```{r crd_dfw}
crd_dfw <- crd_df %>%
  mutate(rep = factor(rep), nrate_kgha = factor(nrate_kgha), krate_kgha=factor(krate_kgha)) %>%
  mutate (trtname = paste0(nrate_kgha, "+", krate_kgha))

#%>% pivot_longer(cols = c(nrate_kgha,krate_kgha),names_to = "factor", values_to = "level")
...
# d) EDA plots  
```{r n boxplot}
ggplot(crd_dfw, aes(x=nrate_kgha, y = yield_kgha, color = nrate_kgha))+
  geom_boxplot()+
  geom_jitter()+
...
```{r }
ggplot(data =crd_resid, aes(x=.fitted,
                            y = .studresid))+
  geom_point(shape =21,
             fill = "purple", size =3, alpha = .7)+
...
crd_means_all <- emmeans::emmeans(crd_mod, ~nrate_kgha:krate_kgha)
...
crd_cld_all <- multcomp::cld(crd_means_all, reversed =T, Letters = letters,
                             adjust="none")  #adjust ="tukey"
...
ggplot()+
  ...
  geom_boxplot(data = crd_dfw, aes(x = trtname, y = yield_kgha))+

  # Adding letters
  geom_label(data = crd_cld_selected, aes(x=trtname, y= emmean, 
                                          label = letter))
